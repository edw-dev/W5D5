...
# Actor
  ...
  Actor.select(:id, :name, "COUNT(castings.id) AS roles").joins(:castings).group('actors.id').order('COUNT(castings.id) DESC').where.not('castings.ord = 1').order('actors.id').limit(2)
  # COUNT(*)
end
